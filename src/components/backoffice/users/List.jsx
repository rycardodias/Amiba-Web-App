...
export const List = () => {
    ...
    //     for (let id of ids) {
    //         const res = await usersRequests.deleteUser(id)
                    ...
                    {/* {selectedRows.length > 0 &&
                        (selectedRows.length === 1 ? (
                            <FlexBox alignItems="center" mr={2}>
                                ...
                                <Button disabled size="small" color="error" variant="contained" endIcon={<Delete />} onClick={handleDelete} sx={{ color: "common.white" }}>
                                    {t('Delete Selected')}
                                </Button>
                            </FlexBox>)
                            : (
