...
const UserInfo = () => {
  ...
  return <Card sx={{ padding: "1.5rem", pb: "4rem" }}>
    ...
    <form onSubmit={handleSubmit}>
      <FlexBox my="1.5rem" flexWrap="wrap" alignItems="center" justifyContent="space-between">
        <FlexBox alignItems="center">
          <StyledBadge overlap="circular" anchorOrigin={{ vertical: "top", horizontal: "right" }} badgeContent={<label htmlFor="icon-button-file">
            ...
            <UkoAvatar alt="Travis Howard" src={user?.avatar || "/static/avatar/001-man.svg"} sx={{ width: 90, height: 90 }} />
          </StyledBadge>
          <Box ml="1rem">
            <H5>{user?.name}</H5>
            <Tiny color="text.disabled">{user?.email}</Tiny>
          </Box>
        </FlexBox>
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={12} sm={6}>
          <LightTextField fullWidth name="location" label="Location" value={values.location} onChange={handleChange} helperText={touched.location && errors.location} error={Boolean(touched.location && errors.location)} />
        </Grid>
        <Grid item xs={12}>
...
const iconStyle = { mr: 1, color: "text.disabled" }; // autocomplete render input function
